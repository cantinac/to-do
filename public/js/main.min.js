...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto), function(a) {
    ...
    a.active = 0, a.ajaxJSONP = function(b, c) {
        ...
        if (n && (i = "jsonp"), c.cache !== !1 && (b && b.cache === !0 || "script" != i && "jsonp" != i) || (c.url = l(c.url, "_=" + Date.now())), 
        "jsonp" == i) return n || (c.url = l(c.url, c.jsonp ? c.jsonp + "=?" : c.jsonp === !1 ? "" : "callback=?")), 
        a.ajaxJSONP(c, e);
    ...
    "object" == typeof exports && "function" == typeof require ? module.exports = b(require("backbone")) : "function" == typeof define && define.amd ? define([ "backbone" ], function(c) {
    ...
    return a.LocalStorage = window.Store = function(a, b) {
        ...
        create: function(a) {
            return a.id || (a.id = c(), a.set(a.idAttribute, a.id)), this.localStorage().setItem(this._itemName(a.id), this.serializer.serialize(a)), 
            this.records.push(a.id.toString()), this.save(), this.find(a) !== !1;
        },
        update: function(a) {
        ...
        findAll: function() {
            for (var a, b, c = [], d = 0; d < this.records.length; d++) a = this.records[d], 
        ...
        h && h.resolve(e)) : (f = f ? f : "Record Not Found", d && d.error && ("0.9.10" === a.VERSION ? d.error(c, f, d) : d.error(f)), 
...
var App = App || {}, TodosCollection = Backbone.Collection.extend({
    model: App.TodoModel,
...
App.TodoView = Backbone.View.extend({
    ...
    render: function() {
        return this.$el.html(this.model.completed ? '<label class="completed"><input type="checkbox" checked />' + this.model.title + "</label>" : '<label><input type="checkbox" />' + this.model.title + "</label>"), 
        this;
    },
    completeTask: function(a) {
        $(a.target.parentElement).addClass("completed"), completed_task = App.Todos.get(this.model.id), 
        completed_task.save({
            completed: !0
        });
    }
...
App.AppView = Backbone.View.extend({
    ...
    initialize: function() {
        App.Todos = new TodosCollection(), App.Todos.fetch({
            forceRefresh: !0
        }), this.addPreviousTasks();
    },
    addTask: function(a) {
        a.preventDefault(), task_title = $("#new-task").val(), todo_view = new App.TodoView({
            model: {
                title: task_title
            }
        }), "" != task_title && ($("#task-list").prepend(todo_view.render().el), App.Todos.create({
            title: $("#new-task").val().trim(),
            completed: !1
        })), $("#new-task").val("");
    },
    addPreviousTasks: function() {
        this.$("#todo-list").html(""), App.Todos.each(function(a) {
            todo_view = new App.TodoView({
                model: a.attributes
